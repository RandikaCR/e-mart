...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
        <div>
            <h4 class="mb-3 mb-md-0">Update Order</h4>
        </div>
    ...
    <div class="row" id="orderWrapper">
        ...
        <div class="col-md-4 grid-margin stretch-card">
            <form class="forms-sample" method="POST"  action="{{ route('orders.update', $order->id ) }}" enctype="multipart/form-data">
                @csrf
                @if(isset($order))
                    @method('PUT')
                @endif
                    ...
                    <div class="card-body" style="min-height: 600px;">
                        ...
                        <div class="row align-items-end  mb-4">
                            <div class="col-md-12">
                                ...
                                <select class="js-example-basic-single form-select" name="status" data-width="100%">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-4 d-flex justify-content-end mb-2 mb-sm-0">
                                <input type="hidden" id="temp_order_id" value="{{ $order->id }}">
                                <button type="submit" class="btn btn-primary me-2 place-order-btn" {{ ( $cart_item_count == 0 ) ? 'disabled' : '' }} >Update Order</button>
                            </div>
                        </div>
                        <hr>
                        <div class="row mb-3">
    ...
    <script>
        $(document).ready(function () {
            ...
            $(wrapper).on('click', '.add-this', function (e) {
                ...
                var id = $(this).data('id');
                var order_id = $('#temp_order_id').val();
                ...
                $.ajax({
                    url: "{{ url('/orders/add-this-to-order') }}",
                    type: "POST",
            ...
            $(wrapper).on('keyup change', '.update-this', function (e) {
                ...
                $.ajax({
                    url: "{{ url('/orders/update-this-to-order') }}",
                    type: "POST",
                    data: {"id":id, "quantity":qty, "order_id":order_id},
                    success: function (data) {
                        $('#order-total-area').html(data.total);
                        $('#items-list').html(data.items);
            ...
            $(wrapper).on('click', '.delete-cart-item', function (e) {
                ...
                custDeleteBtns.fire({
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            url: "{{ url('/orders/delete-this-order-item') }}",
                            method: 'post',
                            data: {
                                id: id
                            },
